...
const VISIBLE_REASONING_PROMPT = [
  ...
  "The current working directory is only the default starting point, not a filesystem boundary.",
  "When the user asks about files elsewhere on the computer, use absolute local paths and explore beyond the project folder when needed.",
  "On Windows, use list_drives first if you need to discover available drive letters.",
  "If the user asks about the whole computer or a folder outside the project and does not give an exact path, proactively inspect drive roots and likely user folders instead of staying in the current working directory.",
  "Do not assume files must be inside the repo. Prefer absolute paths whenever the user is talking about desktop files, downloads, documents, or another disk.",
